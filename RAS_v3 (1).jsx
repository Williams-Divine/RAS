...
import {
  ...
  Volume2, Heart, ChevronDown, Moon, Sun, Megaphone
} from "lucide-react";
...
const LIGHT = {
  bg:"#F6F3EE", bgCard:"#FFFFFF", bgMuted:"#F0EBE3", bgInput:"#FFFFFF",
  primary:"#1A5E8C", primaryL:"#E6F2FA", primaryBtn:"#1A5E8C", primaryBtnTx:"#FFFFFF",
  safe:"#1B7240", safeBg:"rgba(27,114,64,0.09)",
  warn:"#A36C00", warnBg:"rgba(163,108,0,0.09)",
  danger:"#B52C1F", dangerBg:"rgba(181,44,31,0.09)",
  sos:"#B52C1F",
  bdr:"#E4DDD3", bdrMid:"#CEBFB2",
...
const LANGS = [
  ...
  {id:"yo",flag:"YO",name:"Yorùbá"},{id:"ig",flag:"IG",name:"Igbo"},
...
const TOUR = [
  ...
  {Icon:Users,   t:"Save contacts before you need them",b:"Add emergency contacts now. When you send SOS they receive your GPS immediately. Your caregiver is notified first.",    tip:"Add them now, not when you need them."},
];
...
export default function RAS() {
  ...
  const Card=(ex={})=>({background:C.bgCard,borderRadius:16,border:"1px solid "+C.bdr,padding:"16px",...ex});
  const Pill=(active=false)=>({display:"flex",alignItems:"center",gap:6,padding:"9px 14px",borderRadius:100,background:active?C.primaryL:C.bgCard,border:"1px solid "+(active?C.primary:C.bdr),color:active?C.primary:C.tx,fontSize:13,fontWeight:500,cursor:"pointer"});
  const page = {padding:"0 16px 100px",display:"flex",flexDirection:"column",gap:14,animation:"fadeUp .22s ease"};
  ...
  const useGPS=()=>{
    setLocLoad(true);
    if(navigator.geolocation){navigator.geolocation.getCurrentPosition(()=>{const l="Minna, Niger State";setLoc(l);save("ras_loc",l);setLocLoad(false);},()=>{setLocLoad(false);alert("Location access denied. Please type it manually.");},{timeout:6000});}
  ...
  const addContact=()=>{if(!newCon.name||!newCon.phone)return;const n=[...contacts,{...newCon,id:genRef()}];setContacts(n);save("ras_contacts",n);setNewCon({name:"",phone:"",carer:false});};
  const acceptReq=id=>{const req=conReqs.find(r=>r.id===id);if(req){const n=[...contacts,{...req,carer:false}];setContacts(n);save("ras_contacts",n);}setConReqs(p=>p.filter(r=>r.id!==id));};
    ...
    <div style={{position:"fixed",inset:0,background:isDark?"#0F0503":"#1A0503",zIndex:999,display:"flex",flexDirection:"column",padding:"32px 20px 28px",fontFamily:"DM Sans,system-ui,sans-serif",animation:"alertIn .3s ease"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,textAlign:"center"}}>
        ...
        <div style={{background:"rgba(255,255,255,.05)",borderRadius:14,padding:"14px 20px",width:"100%",border:"1px solid rgba(255,255,255,.07)"}}>
          ...
          <p style={{color:"#FFF",fontSize:22,letterSpacing:8,fontWeight:700}}>━━━</p>
          <p style={{color:"rgba(255,255,255,.35)",fontSize:11,marginTop:4}}>3 Long Pulses = Danger Nearby</p>
        </div>
  ...
  if(showZones) return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:90,display:"flex",alignItems:"flex-end",fontFamily:"DM Sans,system-ui,sans-serif"}} onClick={()=>setShowZones(false)}>
      <style>{G}</style>
      <div onClick={e=>e.stopPropagation()} style={{background:C.bgCard,borderRadius:"20px 20px 0 0",padding:"20px 16px 40px",width:"100%",animation:"slideUp .25s ease",color:C.tx}}>
        <div style={{width:40,height:4,borderRadius:2,background:C.bdrMid,margin:"0 auto 20px"}}/>
        <h3 style={{fontSize:18,fontWeight:700,marginBottom:16}}>Nearest Safe Zones</h3>
        {SAFE_ZONES.map(z=>(
          <div key={z.name} style={{...Card({marginBottom:8,display:"flex",gap:12,alignItems:"center"})}}>
            <div style={{width:40,height:40,borderRadius:10,background:C.bgMuted,display:"flex",alignItems:"center",justifyContent:"center"}}><z.Icon size={18} color={C.safe}/></div>
            <div style={{flex:1}}><p style={{fontSize:13,fontWeight:600,color:C.tx}}>{z.name}</p><p style={{fontSize:11,color:C.txs}}>{z.dist}{z.phone?" · "+z.phone:""}</p></div>
            {z.phone&&<button style={{background:C.safeBg,border:"1px solid "+C.safe,color:C.safe,borderRadius:10,padding:"7px 14px",fontSize:12,fontWeight:600,cursor:"pointer"}}>Call</button>}
          </div>
        ))}
  ...
  if(showCall) return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:90,display:"flex",alignItems:"flex-end",fontFamily:"DM Sans,system-ui,sans-serif"}} onClick={()=>setShowCall(false)}>
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:C.bgCard,borderRadius:"20px 20px 0 0",padding:"20px 16px 40px",width:"100%",animation:"slideUp .25s ease",color:C.tx}}>
        ...
        {contacts.length===0&&<p style={{color:C.txs,textAlign:"center",padding:"24px 0",fontSize:14}}>No contacts saved yet.</p>}
        {contacts.map((c,i)=>(
          <div key={i} style={{...Card({marginBottom:8,display:"flex",gap:12,alignItems:"center"})}}>
            <div style={{width:44,height:44,borderRadius:"50%",background:C.bgMuted,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:16,color:C.txs}}>{c.name[0]}</div>
            <div style={{flex:1}}><p style={{fontSize:14,fontWeight:600,color:C.tx}}>{c.name}</p><p style={{fontSize:12,color:C.txs}}>{c.phone}{c.carer?" · Caregiver":""}</p></div>
            <button style={{background:C.safeBg,border:"1px solid "+C.safe,color:C.safe,borderRadius:10,padding:"8px 16px",fontSize:13,fontWeight:600,cursor:"pointer"}}>Call</button>
          </div>
        ))}
        <button onClick={()=>setShowCall(false)} style={GBtn(C.txs,{marginTop:8})}>Close</button>
      </div>
    </div>
  );
  ...
  if(showPhBook) return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:90,display:"flex",alignItems:"flex-end",fontFamily:"DM Sans,system-ui,sans-serif"}} onClick={()=>setShowPB(false)}>
      <style>{G}</style>
      <div onClick={e=>e.stopPropagation()} style={{background:C.bgCard,borderRadius:"20px 20px 0 0",padding:"20px 16px 40px",width:"100%",maxHeight:"70vh",overflowY:"auto",animation:"slideUp .25s ease",color:C.tx}}>
        <div style={{width:40,height:4,borderRadius:2,background:C.bdrMid,margin:"0 auto 20px"}}/>
        <h3 style={{fontSize:18,fontWeight:700,marginBottom:16}}>Select from Contacts</h3>
        {PHONEBOOK.map(c=>(
          <button key={c.name} onClick={()=>{setNewCon({name:c.name,phone:c.phone,carer:false});setShowPB(false);}}
  ...
  if(showAllAnn) return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:90,display:"flex",alignItems:"flex-end",fontFamily:"DM Sans,system-ui,sans-serif"}} onClick={()=>setShowAllAnn(false)}>
      <style>{G}</style>
      <div onClick={e=>e.stopPropagation()} style={{background:C.bgCard,borderRadius:"20px 20px 0 0",padding:"20px 16px 40px",width:"100%",maxHeight:"85vh",overflowY:"auto",animation:"slideUp .25s ease",color:C.tx}}>
        <div style={{width:40,height:4,borderRadius:2,background:C.bdrMid,margin:"0 auto 20px"}}/>
        ...
        {ANNOUNCEMENTS.map(an=>(
          <div key={an.id} style={{...Card({marginBottom:12,border:"1px solid "+SEV[an.sev].c+"22"})}}>
            ...
            {an.endsAt&&cd&&(
              <div style={{background:C.dangerBg,borderRadius:10,padding:"10px 14px",marginBottom:10}}>
                <p style={{fontSize:10,color:C.danger,fontWeight:700}}>CURFEW {an.phase==="ending"?"ENDS IN":"STARTS IN"}</p>
                <p style={{fontSize:24,fontWeight:800,color:C.danger,fontVariantNumeric:"tabular-nums"}}>{cd}</p>
              </div>
            )}
  ...
  case "onboard": return (
    <div style={{minHeight:"100vh",background:C.bg,padding:"28px 20px",color:C.tx}}>
      ...
      {ob.step===0&&(
        <div style={{animation:"fadeUp .22s ease",display:"flex",flexDirection:"column",gap:16}}>
          <h2 style={{fontSize:26,fontWeight:800,color:C.tx,marginBottom:4}}>Choose language</h2>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {LANGS.map(l=>(
              <button key={l.id} onClick={()=>setLang(l.id)}
                style={{padding:"16px 14px",borderRadius:14,border:"2px solid "+(lang===l.id?C.primary:C.bdr),background:lang===l.id?C.primaryL:C.bgCard,cursor:"pointer",display:"flex",alignItems:"center",gap:10}}>
      ...
      {ob.step===1&&(
        <div style={{animation:"fadeUp .22s ease",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div>
            <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:8}}>YOUR NAME</p>
            <input style={Inp()} value={ob.data.name} onChange={e=>setOb(p=>({...p,data:{...p.data,name:e.target.value}}))} placeholder="e.g. Amina Bello"/>
          </div>
          <div>
            <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:8}}>PHONE NUMBER</p>
            <input style={Inp()} type="tel" value={ob.data.phone} onChange={e=>setOb(p=>({...p,data:{...p.data,phone:e.target.value}}))} placeholder="+234 803 000 0000"/>
            <p style={{fontSize:12,color:C.txs,marginTop:6}}>Used for SMS alerts and SOS when offline.</p>
          </div>
          <button onClick={()=>setOb(p=>({...p,step:2}))} disabled={!ob.data.phone}
            style={PBtn({background:ob.data.phone?C.primaryBtn:"#ccc",color:ob.data.phone?C.primaryBtnTx:"#888"})}>Continue</button>
      ...
      {ob.step===2&&(
        <div style={{animation:"fadeUp .22s ease",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{...Card({padding:"28px 20px",minHeight:260,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,textAlign:"center"})}}>
            ...
            <div style={{background:C.bgMuted,borderRadius:10,padding:"8px 14px"}}>
              <p style={{fontSize:12,fontWeight:600,color:C.txs}}>{TOUR[ob.tour].tip}</p>
            </div>
          </div>
          ...
          {ob.tour<TOUR.length-1
            ?<button onClick={()=>setOb(p=>({...p,tour:p.tour+1}))} style={PBtn()}>Next</button>
            :<button onClick={()=>setOb(p=>({...p,step:3}))} style={PBtn()}>Got it</button>}
        </div>
      )}
      ...
      {ob.step===3&&(
        <div style={{animation:"fadeUp .22s ease",display:"flex",flexDirection:"column",gap:16}}>
          ...
          <div style={{...Card({border:"1px solid "+C.primary+"22",background:C.primaryL,padding:"12px 14px"})}}>
            <p style={{fontSize:12,color:C.primary}}>Your location is attached to every SOS you send.</p>
          </div>
          <div>
            <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:8}}>CITY, LGA OR AREA</p>
            <input style={Inp()} value={loc} onChange={e=>{setLoc(e.target.value);save("ras_loc",e.target.value);}} placeholder="e.g. Minna, Niger State"/>
          </div>
          <button onClick={useGPS} disabled={locLoad}
            style={{background:"none",border:"1.5px solid "+C.primary,borderRadius:14,padding:"13px 20px",color:C.primary,fontSize:14,fontWeight:600,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
            <MapPin size={16}/>{locLoad?"Detecting your location...":"Use my current location"}
          </button>
          <button onClick={()=>setOb(p=>({...p,step:4}))}
            style={PBtn({background:loc?C.primaryBtn:"#ccc",color:loc?C.primaryBtnTx:"#888"})}>Continue</button>
      ...
      {ob.step===4&&(
        <div style={{animation:"fadeUp .22s ease",display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:4}}>
            <div><h2 style={{fontSize:22,fontWeight:800,color:C.tx,marginBottom:4}}>Any accessibility needs?</h2>
              <p style={{fontSize:14,color:C.txs}}>Tap the arrow to see what changes.</p></div>
            ...
            return(
              <div key={d.id} style={{display:"flex",gap:0}}>
                <button onClick={()=>{if(d.id==="none"){setOb(p=>({...p,disab:[]}));}else{setOb(p=>({...p,disab:p.disab.includes(d.id)?p.disab.filter(x=>x!==d.id):[...p.disab.filter(x=>x!=="none"),d.id]}));}}}
                  ...
                  <div style={{width:20,height:20,borderRadius:"50%",border:"2px solid "+(sel?C.primary:C.bdrMid),background:sel?C.primary:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    {sel&&<Check size={11} color="#FFF"/>}
                  </div>
                </button>
                <button onClick={()=>setShowDF(d.id)} style={{padding:"13px 12px",borderRadius:"0 12px 12px 0",border:"1.5px solid "+(sel?C.primary:C.bdr),borderLeft:"none",background:sel?C.primaryL:C.bgCard,cursor:"pointer",color:C.txs}}>
  ...
  case "home": if(simple) return (
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",padding:"28px 20px 100px",gap:14,color:C.tx}}>
      <style>{G}</style>
      <p style={{fontSize:15,color:C.txs}}>{greeting()}, {profile?.name}</p>
      {[{Icon:Map,l:"Check Safety",a:()=>go("route"),c:C.warn},{Icon:AlertCircle,l:"I Need Help",a:initSOS,c:C.danger},{Icon:FileText,l:"Report Incident",a:()=>go("report"),c:C.primary}].map(b=>(
        <button key={b.l} onClick={b.a} style={{...Card({display:"flex",alignItems:"center",gap:16,cursor:"pointer",flex:1,padding:"24px 20px",border:"1px solid "+b.c+"22"})}}>
          <div style={{width:48,height:48,borderRadius:14,background:b.c+"10",display:"flex",alignItems:"center",justifyContent:"center"}}><b.Icon size={26} color={b.c}/></div>
          <span style={{fontSize:18,fontWeight:700,color:C.tx}}>{b.l}</span>
        </button>
      ))}
      <button onClick={()=>setSimple(false)} style={GBtn(C.txs,{fontSize:13})}>Switch to full view</button>
    </div>
  );
  return (
    <div style={{...page,paddingTop:0,color:C.tx}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"20px 0 4px"}}>
        <div>
          ...
          <h2 style={{fontSize:22,fontWeight:800,color:C.tx}}>{profile?.name}</h2>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={toggleTheme} style={{background:C.bgCard,border:"1px solid "+C.bdr,borderRadius:12,width:40,height:40,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
            {isDark?<Sun size={17} color={C.txs}/>:<Moon size={17} color={C.txs}/>}
          </button>
          <button onClick={()=>setShowAlert(true)} style={{background:C.bgCard,border:"1px solid "+C.bdr,borderRadius:12,width:40,height:40,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}><Bell size={17} color={C.warn}/></button>
        </div>
      </div>
      <button onClick={()=>go("profile")} style={{...Card({display:"flex",alignItems:"center",gap:8,cursor:"pointer",padding:"10px 14px",border:"none"})}}>
        <MapPin size={15} color={loc?C.primary:C.txm}/>
        <span style={{fontSize:13,color:loc?C.tx:C.txm,flex:1}}>{loc||"Tap to set your location — critical for SOS"}</span>
        ...
        <div style={{...Card({border:"1.5px solid "+(jOverdue?C.danger:C.primary),background:jOverdue?C.dangerBg:C.primaryL})}}>
          <div style={{display:"flex",gap:12,alignItems:"center",marginBottom:10}}>
            ...
            <div style={{flex:1}}>
              ...
              <p style={{fontSize:13,color:C.tx}}>En route to {journey.dest.name} · {fmtJTime(jElapsed)}</p>
            </div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <button onClick={()=>setJOverdue(false)} style={PBtn({flex:1,padding:"10px",fontSize:13})}>Check In</button>
      ...
      {ANNOUNCEMENTS.filter(a=>a.sev==="danger").slice(0,1).map(an=>(
        <button key={an.id} onClick={()=>setShowAllAnn(true)}
          style={{background:"transparent",border:"none",padding:0,cursor:"pointer",textAlign:"left",width:"100%"}}>
          <div style={{background:C.dangerBg,borderLeft:"5px solid "+C.danger,borderRadius:16,padding:"18px 16px",animation:"bannerPulse 4s infinite",border:"1px solid "+C.danger+"22",borderLeftWidth:5}}>
            <div style={{display:"flex",gap:14,alignItems:"flex-start"}}>
              <div style={{width:44,height:44,borderRadius:14,background:C.danger+"22",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><AlertTriangle size={22} color={C.danger}/></div>
              <div style={{flex:1}}>
                <p style={{fontSize:10,fontWeight:700,color:C.danger,letterSpacing:1.2,marginBottom:4}}>CURFEW IN EFFECT</p>
                <p style={{fontSize:16,fontWeight:700,color:C.tx,marginBottom:6,lineHeight:1.2}}>{an.title}</p>
                {cd&&(
                  <div>
                    <p style={{fontSize:11,color:C.txs,marginBottom:2}}>Curfew {an.phase==="ending"?"ends":"starts"} in</p>
                    <p style={{fontSize:30,fontWeight:800,color:C.danger,lineHeight:1,fontVariantNumeric:"tabular-nums"}}>{cd}</p>
                  </div>
                )}
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:6,marginTop:14,color:C.danger}}>
              <span style={{fontSize:12,fontWeight:600}}>View all announcements</span>
      ...
      {!contacts.length&&(
        <button onClick={()=>go("contacts")} style={{...Card({display:"flex",gap:12,alignItems:"center",cursor:"pointer",textAlign:"left",width:"100%",border:"1px dashed "+C.bdrMid,padding:"12px 14px"})}}>
          <Users size={18} color={C.txm}/>
          <div style={{flex:1}}><p style={{fontSize:13,fontWeight:600,color:C.tx}}>Add emergency contacts</p><p style={{fontSize:12,color:C.txs}}>They get your GPS the moment you send SOS.</p></div>
          <span style={{fontSize:12,color:C.primary,fontWeight:600}}>Add</span>
        </button>
      )}

      {/* Quick action pills — WRAP not scroll */}
      <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
        {[{Icon:Map,l:"Check Route",a:()=>go("route")},{Icon:Navigation,l:"Start Journey",a:()=>go("journey-setup")},{Icon:FileText,l:"Report",a:()=>go("report")},{Icon:Megaphone,l:"Announcements",a:()=>setShowAllAnn(true)},{Icon:Users,l:"Contacts",a:()=>go("contacts")},{Icon:FileText,l:"My Reports",a:()=>go("my-reports")}].map(p=>(
          <button key={p.l} onClick={p.a} style={Pill()}><p.Icon size={13}/>{p.l}</button>
        ))}
      </div>

      <div style={Card()}>
        <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:6}}>AREA STATUS</p>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><p style={{fontSize:17,fontWeight:700,color:C.tx}}>{loc||"Set your location"}</p><p style={{fontSize:13,color:C.warn,marginTop:2}}>3 active reports nearby</p></div>
          <SBadge s="caution"/>
        </div>
      </div>
      ...
      <button onClick={()=>setShowRcv(true)} style={{...Card({display:"flex",gap:12,alignItems:"center",cursor:"pointer",textAlign:"left",width:"100%",border:"none",background:C.warnBg,padding:"12px 14px"})}}>
        <Bell size={18} color={C.warn}/>
        <div style={{flex:1}}><p style={{fontSize:13,fontWeight:600,color:C.warn}}>Simulate: Receive an SOS</p><p style={{fontSize:11,color:C.txs}}>Experience being someone's emergency contact.</p></div>
      </button>
    </div>
  );
  ...
  case "route": return (
    <div style={{...page,paddingTop:0,color:C.tx}}>
      ...
      <div>
        ...
        {MAP_NODES.map(n=>(
          <button key={n.id} onClick={()=>setMapSel(mapSel?.id===n.id?null:n)}
            style={{display:"flex",gap:12,alignItems:"center",width:"100%",padding:"12px 4px",border:"none",borderBottom:"1px solid "+C.bdr,background:"transparent",cursor:"pointer",textAlign:"left"}}>
  ...
  case "journey-setup": return (
    <div style={{...page,color:C.tx}}>
      ...
      <p style={{fontSize:14,color:C.txs,lineHeight:1.6}}>RAS actively monitors your route. Check in every 30 minutes. Miss a check-in and your contacts are alerted.</p>
      {jSetup?(
        ...
        <div>
          <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:8}}>SELECT DESTINATION</p>
      ...
      <button onClick={()=>{if(jSetup)startJourney(jSetup);else alert("Please select a destination.");}}
        style={PBtn({background:jSetup?C.primaryBtn:"#ccc",color:jSetup?C.primaryBtnTx:"#888"})}>Start Journey</button>
      <button onClick={()=>go("home")} style={GBtn()}>Cancel</button>
    </div>
  );
  ...
  case "report": return (
    <div style={{...page,color:C.tx}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        {INCIDENTS.map(inc=>(
          <button key={inc.id} onClick={()=>{setRpType(inc.id);go("report-detail");}}
            style={{...Card({display:"flex",flexDirection:"column",alignItems:"center",gap:10,padding:"20px 12px",cursor:"pointer",border:"none",textAlign:"center"})}}>
  ...
  case "report-detail": return (
    <div style={{...page,color:C.tx}}>
      ...
      <div style={Card()}><p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:8}}>LOCATION</p><input style={Inp()} value={rpLoc||loc} onChange={e=>setRpLoc(e.target.value)} placeholder="Area, LGA, or nearby landmark"/></div>
      <div style={Card()}><p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:8}}>WHAT HAPPENED?</p><textarea value={rpNote} onChange={e=>setRpNote(e.target.value)} placeholder="Vehicles, direction, count..." style={{...Inp({resize:"vertical",minHeight:80})}}/>
      </div>
      <div style={Card()}>
        <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:8}}>VOICE NOTE (OPTIONAL)</p>
        {!rpAudio?(isRec?(
          ...
          <button onClick={startRec} style={{...GBtn(C.txs,{display:"flex",alignItems:"center",gap:8,justifyContent:"center"})}}>
  ...
  case "report-done": return (
    <div style={{...page,alignItems:"center",textAlign:"center",color:C.tx}}>
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
        ...
        <div style={{...Card({border:"1px solid "+C.primary+"22",background:C.primaryL,padding:"12px 24px"})}}>
          <p style={{fontSize:11,color:C.txm,marginBottom:4}}>Reference</p>
          <p style={{fontSize:18,fontWeight:700,color:C.primary,letterSpacing:2}}>{rpRef}</p>
        </div>
  ...
  case "sos": return (
    <div style={{minHeight:"100vh",background:isDark?"#080303":"#FFF5F5",display:"flex",flexDirection:"column",padding:"24px 20px",gap:14,fontFamily:"DM Sans,system-ui,sans-serif",color:C.tx}}>
      <style>{G}</style>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <button onClick={()=>{clearInterval(sosTimRef.current);go("home");}} style={{background:"none",border:"none",color:C.txs,cursor:"pointer",display:"flex",alignItems:"center",gap:6,fontSize:15}}>
          <X size={18}/>Cancel
        </button>
        ...
        <>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:14,flex:1,justifyContent:"center"}}>
            ...
            <div style={{position:"relative",width:180,height:180}}>
              ...
              <button onClick={()=>fireSOS(sosType||"other")}
                style={{position:"absolute",inset:12,borderRadius:"50%",background:C.sos,border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",animation:"glow 1.5s infinite"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {SOS_OPTS.map(s=>(
              <button key={s.id} onClick={()=>fireSOS(s.id)}
                style={{...Card({display:"flex",flexDirection:"column",alignItems:"center",gap:8,padding:"16px 10px",cursor:"pointer",border:"1px solid "+(sosType===s.id?C.danger:C.bdr),background:sosType===s.id?C.dangerBg:C.bgCard,textAlign:"center"})}}>
      ...
      {sosStep===2&&(
        <div style={{flex:1,display:"flex",flexDirection:"column",gap:14,paddingTop:8}}>
          ...
          <div style={Card()}>
            {[contacts.length+"+ contacts notified","112 Emergency Services alerted","GPS location shared — "+(loc||"last known area"),disab.includes("deaf")?"Responders told: use text only":"",contacts.find(c=>c.carer)?"Caregiver "+contacts.find(c=>c.carer).name+" notified first":""].filter(Boolean).map((l,i)=>(
              <div key={i} style={{display:"flex",gap:10,alignItems:"center",marginBottom:8}}>
                <Check size={14} color={C.safe}/><span style={{fontSize:13,color:C.tx}}>{l}</span>
              </div>
            ))}
  ...
  case "profile": return (
    <div style={{...page,color:C.tx}}>
      ...
      <div style={Card()}>
        <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:8}}>YOUR LOCATION</p>
        <input style={Inp()} value={loc} onChange={e=>{setLoc(e.target.value);save("ras_loc",e.target.value);}} placeholder="e.g. Minna, Niger State"/>
        <button onClick={useGPS} style={{background:"none",border:"none",color:C.primary,fontSize:12,fontWeight:600,cursor:"pointer",padding:"6px 0",marginTop:6,display:"flex",alignItems:"center",gap:6}}>
          <MapPin size={12}/>Use current location
        </button>
      </div>
      {[{Icon:Activity,l:"Accessibility",sub:disab.length+" option"+(disab.length!==1?"s":"")+" active",a:()=>go("accessibility")},{Icon:Users,l:"Emergency Contacts",sub:contacts.length+" saved · "+conReqs.length+" pending",a:()=>go("contacts")},{Icon:FileText,l:"My Reports",sub:reports.length+" submitted",a:()=>go("my-reports")},{Icon:HelpCircle,l:"Help & About",sub:null,a:()=>go("help")}].map(it=>(
        <button key={it.l} onClick={it.a} style={{...Card({display:"flex",gap:14,alignItems:"center",cursor:"pointer",border:"none",width:"100%",textAlign:"left"})}}>
          <div style={{width:38,height:38,borderRadius:10,background:C.bgMuted,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><it.Icon size={18} color={C.txs}/></div>
          <div style={{flex:1}}><p style={{fontSize:14,fontWeight:500,color:C.tx}}>{it.l}</p>{it.sub&&<p style={{fontSize:11,color:C.txs,marginTop:1}}>{it.sub}</p>}</div>
          <ChevronRight size={16} color={C.txm}/>
        </button>
      ))}
      <div style={Card()}>
        <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:10}}>LANGUAGE</p>
        <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
          {LANGS.map(l2=>(
            <button key={l2.id} onClick={()=>{setLang(l2.id);save("ras_lang",l2.id);}}
              style={{padding:"8px 14px",borderRadius:10,border:"1.5px solid "+(lang===l2.id?C.primary:C.bdr),background:lang===l2.id?C.primaryL:"transparent",color:lang===l2.id?C.primary:C.tx,fontWeight:lang===l2.id?600:400,cursor:"pointer",fontSize:13}}>
              {l2.name}
            </button>
          ))}
        </div>
      ...
      <button onClick={()=>{if(window.confirm("Reset all data?"))[...["ras_profile","ras_disab","ras_contacts","ras_reports","ras_simple","ras_lang","ras_loc","ras_dark"]].forEach(k=>{try{window.storage.delete(k);}catch{}});window.location.reload();}}
        style={{fontSize:13,color:C.danger,background:"none",border:"1px solid "+C.danger+"22",borderRadius:10,padding:10,cursor:"pointer"}}>Reset & Start Over</button>
    </div>
  );
  ...
  case "accessibility": return (
    <div style={{...page,color:C.tx}}>
      ...
      <p style={{fontSize:13,color:C.txs,lineHeight:1.6}}>Tap the arrow to see what changes per option.</p>
      {DISABILITIES.map(d=>{
      ...
      <div style={Card()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><p style={{fontSize:14,fontWeight:500,color:C.tx}}>Simple Mode</p><p style={{fontSize:11,color:C.txs}}>3 buttons only</p></div>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
  ...
  case "contacts": return (
    <div style={{...page,color:C.tx}}>
      ...
      {conReqs.length>0&&(
        <div style={{...Card({border:"1px solid "+C.warn+"22",background:C.warnBg})}}>
          <p style={{fontSize:11,fontWeight:700,color:C.warn,marginBottom:10,letterSpacing:.8}}>PENDING REQUESTS</p>
          {conReqs.map(r=>(
            <div key={r.id} style={{display:"flex",gap:10,alignItems:"center",marginBottom:8}}>
              <div style={{flex:1}}><p style={{fontSize:13,fontWeight:600,color:C.tx}}>{r.name} wants to add you as their contact</p><p style={{fontSize:11,color:C.txs}}>{r.ago}</p></div>
              <button onClick={()=>acceptReq(r.id)} style={{background:C.safeBg,border:"1px solid "+C.safe,color:C.safe,borderRadius:8,padding:"6px 12px",fontSize:12,fontWeight:600,cursor:"pointer"}}>Accept</button>
              <button onClick={()=>setConReqs(p=>p.filter(x=>x.id!==r.id))} style={{background:"none",border:"1px solid "+C.bdr,color:C.txm,borderRadius:8,padding:"6px 10px",fontSize:12,cursor:"pointer"}}>X</button>
            </div>
          ))}
        </div>
      )}
      {contacts.map((c,i)=>(
        <div key={i} style={{...Card({display:"flex",gap:12,alignItems:"center"})}}>
          <div style={{width:44,height:44,borderRadius:"50%",background:C.bgMuted,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:16,color:C.txs}}>{c.name[0]}</div>
          <div style={{flex:1}}><p style={{fontSize:14,fontWeight:600,color:C.tx}}>{c.name}</p><p style={{fontSize:12,color:C.txs}}>{c.phone}{c.carer?" · Caregiver":""}</p></div>
          <button onClick={()=>{const n=contacts.filter((_,j)=>j!==i);setContacts(n);save("ras_contacts",n);}} style={{background:"none",border:"none",color:C.txm,cursor:"pointer",fontSize:20,padding:4}}>x</button>
        </div>
      ))}
      {contacts.length===0&&(<div style={{...Card({textAlign:"center",padding:"32px"})}}><Users size={32} color={C.txm} style={{margin:"0 auto 12px"}}/><p style={{fontSize:14,color:C.txs}}>No emergency contacts yet.</p></div>)}
      <div style={Card()}>
        <p style={{fontSize:11,fontWeight:700,color:C.txm,letterSpacing:.8,marginBottom:12}}>ADD CONTACT</p>
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          <input style={Inp({flex:1})} value={newCon.name} onChange={e=>setNewCon(p=>({...p,name:e.target.value}))} placeholder="Full name"/>
          <button onClick={()=>setShowPB(true)} style={{background:C.bgMuted,border:"1px solid "+C.bdr,borderRadius:12,padding:"0 14px",cursor:"pointer",color:C.txs,flexShrink:0}}><Smartphone size={16}/></button>
        </div>
        <input style={Inp({marginBottom:10})} type="tel" value={newCon.phone} onChange={e=>setNewCon(p=>({...p,phone:e.target.value}))} placeholder="+234 803 000 0000"/>
        <label style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",fontSize:13,color:C.txs,marginBottom:12}}>
          <input type="checkbox" checked={newCon.carer||false} onChange={e=>setNewCon(p=>({...p,carer:e.target.checked}))} style={{accentColor:C.primary,width:16,height:16}}/>
          This person is my caregiver (notified first)
        </label>
        <button onClick={addContact} disabled={!newCon.name||!newCon.phone} style={PBtn({background:newCon.name&&newCon.phone?C.primaryBtn:"#ccc",color:newCon.name&&newCon.phone?C.primaryBtnTx:"#888"})}>Save Contact</button>
      </div>
      <button onClick={()=>setShowRcv(true)} style={GBtn(C.txs)}>Simulate receiving an SOS (demo)</button>
    </div>
  );
  ...
  case "my-reports": return (
    <div style={{...page,color:C.tx}}>
      ...
      {reports.length===0?(<div style={{...Card({textAlign:"center",padding:"40px 20px"})}}><FileText size={36} color={C.txm} style={{margin:"0 auto 14px"}}/><p style={{fontSize:14,color:C.txs}}>No reports yet.</p><button onClick={()=>go("report")} style={PBtn({marginTop:16})}>Report an Incident</button></div>):(reports.map(r=>(
        <div key={r.id} style={Card()}>
          <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
            <div style={{width:40,height:40,borderRadius:12,background:C.bgMuted,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              {INCIDENTS.find(i=>i.id===r.type)&&(()=>{const Ic=INCIDENTS.find(i=>i.id===r.type)?.Icon||FileText;return <Ic size={18} color={C.txs}/>;})()}
            </div>
  ...
  case "help": return (
    <div style={{...page,color:C.tx}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <button onClick={()=>go("profile")} style={{background:"none",border:"none",cursor:"pointer",color:C.txs}}><ArrowLeft size={22}/></button>
        <h2 style={{fontSize:18,fontWeight:700,color:C.tx}}>Help & About</h2>
      </div>
      <div style={{...Card({textAlign:"center",padding:"24px",background:C.primaryL,border:"1px solid "+C.primary+"22"})}}><Shield size={36} color={C.primary} style={{margin:"0 auto 10px"}}/><h2 style={{fontSize:20,fontWeight:800,color:C.tx}}>RAS</h2><p style={{color:C.txs,fontSize:12,marginTop:4}}>Rapid Alert System · Niger State · v3.1</p></div>
      {[{q:"How does SOS work without internet?",a:"RAS tries the internet first. If unavailable, it auto-sends via SMS to your contacts and 112. You only need a basic mobile signal."},{q:"Are my reports anonymous?",a:"Yes. Your name and phone are never shown publicly. Only the platform knows, for abuse prevention — never shared."},{q:"What is Start a Journey?",a:"RAS monitors your route while you travel. Check in every 30 minutes. Miss a check-in and your emergency contacts are alerted with your last location."},{q:"What does the vibration pattern mean?",a:"3 Long pulses: Danger. 3 Short: Caution. Long-Short-Long: Evacuate. 1 Short: All clear."},{q:"Can I use RAS on a basic phone?",a:"Yes. Alerts work via SMS. SOS works via SMS. A USSD version is available for feature phones."}].map(({q,a},i)=>(
  ...
  return (
    <div style={{maxWidth:440,margin:"0 auto",minHeight:"100vh",background:C.bg,fontFamily:"DM Sans,system-ui,sans-serif",color:C.tx,position:"relative"}}>
      <style>{G}</style>
      {render()}
      {showNav&&(
